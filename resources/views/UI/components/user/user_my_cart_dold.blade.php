...
<div class="block">
	<div class="container"> 
		<div class="row cart">
			...
			@if(Auth::check())
               	@if(is_array($result))
					<div class="col-sm-12 col-md-12 col-lg-8 cart__table cart-table">
                      ...
                      @endif
										...
										<tr class="cart-table__row" id="record-{{$details->id}}">
											<td class="cart-table__column cart-table__column--image">
												<a href="{{url('/package-detail/'.$details->package_id)}}">
													<img src="{{$details->image}}" alt="">
												</a>
											</td>
											<td class="cart-table__column cart-table__column--product"><a href="{{url('/package-detail/'.$details->package_id)}}" class="cart-table__product-name">	{{ $details->package_name }}</a> 
											</td>
											<td class="cart-table__column cart-table__column--price" data-title="Price"><i class="fas fa-rupee-sign"></i>
												<span id="price-{{$details->id}}">
							...
							<tfoot class="cart-table__foot">
								<tr>
									<td colspan="6">
                                    	...
                                    	@if($coupon_error=='2' || $coupon_error=='3')
                                                <div class="alert alert-danger alert-dismissible" id="coupon_error">
					...
					<div class="col-sm-12 col-md-12 col-lg-4 cart__totals">
						<div class="card">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												...
												@endif
                                            <form class="cart-table__coupon-form form-row" action="{{url('remove-my-cart-coupon')}}" method="post">
                                            <input type="hidden" name="remove" value="1">
                                            <input type="hidden" value="{{csrf_token()}}" name="_token" id="_token">
                                            <button class="btn"  style="position:absolute; top: 112px; left: 339px;" title="Remove Coupon" type="submit"><i class="fa fa-trash"></i></button>
                                            </form> 
											</td>
										</tr> 
										@endif 
							...
							<tbody class="cart-table__body">
                                ...
                                @foreach($result as  $details)
    								@if($details->type == 1)
                                        ...
                                        @if($details->special_price != null &&  isset($details->extra_discount) && $details->extra_discount != null) 
												<?php
												$extra_discount_1 = ($details->special_price * $details->quantity *  $details->extra_discount)/100; 
												?>
												@elseif(isset($details->price) && $details->price != null && $details->extra_discount != null) 
												<?php
												$extra_discount_1 = ($details->price * $details->quantity *  $details->extra_discount)/100; 
												?>
												@endif 
								...
								@php  $details->id = !empty($details->id) ? $details->id : $details->temp_carts_id; @endphp
									@if(isset($details->products_id) && ($details->type == 1 || $details->type == 2))
										<?php 
											$category = DB::table('product_images')->where('type',2)->where('products_id' , $details->products_id)->pluck('product_image')->first();  
										?>
										<tr class="cart-table__row" id="record-{{$details->id}}">
					...
					<div class="col-sm-12 col-md-12 col-lg-4 cart__totals">
						<div class="card">
							<div class="card-body card-body--padding--1">
								...
								@else
									<div class="text-center">
										<a class="btn btn-primary" href="{{url('/guest-user')}}">Proceed to checkout</a>
									</div>
								@endif
							</div>
						</div>
