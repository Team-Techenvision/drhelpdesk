...
<div class="col-md-6"> 
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title">Edit Products Attributes</h3>			
		...
		<form action="{{url('products-attribute-update')}}" method="post"  enctype="multipart/form-data">
			{{ csrf_field() }}  
			...
			<div class="box-body">
				...
				<div class="form-group">
					<label>Product Have Multiple Attributes</label> 
                ...
                <div class="form-group">
					...
					<select class="form-control" name="product_size" id="product_size">
						<!-- <option value="">select</option> -->
						@foreach($size as $r) 
							<option value="{{$r->id}}" @if($r->id == $result->product_size)selected @endif >{{$r->size_name}}</option>
							@error('parent_id')
		                     	<p>* {{ $message }}</p>
		                	@enderror
						@endforeach
				...
				<div class="form-group" id="stript_quantity">
					...
					<input type="number" class="form-control" name="per_stript_qty" min="1" value="{{$result->per_stript_qty}}">  
				</div> 
                ...
                <div class="form-group show-color-div">
					...
					<input type="color" class="form-control" name="product_color" value="{{$result->product_color}}">  
				</div> 

				<div class="form-group">
					<label>Product Barcode</label>
					<input type="text" class="form-control" name="barcode" value="{{$result->barcode}}" readonly> 
					@error('barcode')
                     	<p>* {{ $message }}</p>
                	@enderror
				</div>
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="manufacturer_price" value="{{$result->manufacturer_price }}" id="price-from" required> 
				</div> 

				<div class="form-group">
					<label>MRP <span class="text-danger">*</span></label>
					<input type="text" class="form-control" name="price"  value="{{$result->price}}" required> 
					@error('price')
                     	<p>* {{ $message }}</p>
                	@enderror
				</div>   
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="shop_price" value="{{$result->shop_price}}"> 
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<input type="number" class="form-control" name="quantity" value="{{$result->quantity}}" required>  
				</div> 
				...
				<div class="form-group">
					...
					<select class="form-control" name="status" required>
						<option value="">select</option>						 
							<option value="1" @if( '1' == $result->status )selected @endif>Active</option>	
							<option value="0" @if( '0' == $result->status )selected @endif>Inactive</option>					
					</select> 
				</div> 
