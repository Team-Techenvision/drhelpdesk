		...
		<form action="{{url('products-submit')}}" method="post"  enctype="multipart/form-data">
			...
			<div class="box-body"> 
				...
				<div class="form-group">
					...
					<option value="{{$r->categories_id}}">{{$r->sub_category_name}} <small> ({{$r->main_cate}})</small> </option>
				...
				<div class="form-group">
					...
					<select class="form-control" name="sub_sub_categories">
						...
						@foreach($sub_sub_category as $r1) 
							<option value="{{$r1->categories_id}}">{{$r1->sub_category_name}} <small> ({{$r->main_cate}})</small></option> 
						@endforeach
					</select> 
				</div>
				<div class="form-group">
