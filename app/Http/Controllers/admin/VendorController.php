...
class VendorController extends Controller
{
    ...
    // product start   
        public function addProduct(){
            ...
            $data['sub_category'] = Category::Join('categories as main_category','main_category.categories_id','categories.parent_id')->select('categories.*','main_category.category_name as main_cate')->where('categories.category_name', null)->where('categories.sub_category_name',  '!=' , null)->where('categories.parent_id',  '!=' , null)->where('categories.sub_parent_id',null)->where('categories.sub_sub_parent_id',null)->where('categories.status',0)->orderBy('categories.sub_category_name','asc')->get();    
            ...
            $data['sub_sub_category'] = Category::Join('categories as main_category','main_category.categories_id','categories.parent_id')->select('categories.*','main_category.category_name as main_cate')->where('categories.category_name', null)->where('categories.sub_category_name',  '!=' , null)->where('categories.parent_id',  '!=' , null)->where('categories.sub_parent_id',  '!=' , null)->where('categories.sub_sub_parent_id',null)->where('categories.status',0)->orderBy('categories.sub_category_name','asc')->get(); 
        ...
        public function productattributeSubmit(Request $req){        	
            ...
            if($req->shop_price){
            $price_per_pic = round(($req->shop_price/$req->per_stript_qty), 3);
            }else{
     ...
     */
     ...
     public function update_size(Request $req){
        ...
        public function  deleteProduct($products_id){ 
            ...
            $product_info = DB::table('products')->join('product_attributes', 'product_attributes.products_id', '=', 'products.products_id')->where('products.products_id',$products_id)->first();
            // dd($product_info);
            $data = new Product_Deleted();
            $data->products_id = $product_info->products_id;
            $data->product_name = $product_info->product_name;
            $data->product_code = $product_info->product_code;
            $data->attribute_id = $product_info->id;
            $data->barcode = $product_info->barcode;
            $data->save();

            $data['result']=Product::where('products_id',$products_id)->delete();
            $data['result']=ProductAttribute::where('products_id',$products_id)->delete();
        ...
        public function productSubmit(Request $req){    
            ...
            if($req->products_id){ 
                ...
                if($req->hasFile('product_image_one')) {
                    ...
                    $data1->products_id = $req->products_id; 
