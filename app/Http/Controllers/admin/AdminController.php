 ...
 //new code 6/2/2021
    ...
    */
    ...
    public function Increment_Tax()
    {
        ...
        // $data['increment_tax'] = DB::select("SELECT products.products_id,products.product_name,sizes.size_name,gst_tax.gst_value_percentage AS GST ,order_items.sub_total  AS Sell_Price,p_attr.manufacturer_price AS M_price,shop_stocks.input_quantity AS Quantity,SUM(order_items.quantity) AS Sell_QTY FROM order_items inner JOIN orders ON(orders.order_id=order_items.order_id)inner JOIN product_attributes p_attr ON (p_attr.id=order_items.prod_id)inner JOIN sizes ON(sizes.id=p_attr.product_size)inner JOIN products ON(products.products_id=p_attr.products_id)inner JOIN gst_tax ON(gst_tax.gst_id=products.gst_id)inner JOIN shop_stocks ON(shop_stocks.shop_id=orders.shop_id) WHERE (orders.shop_id=101) GROUP BY(order_items.prod_id)"); 
    //    $data['increment_tax'] = DB::select("SELECT products.products_id,products.product_name,sizes.size_name,gst_tax.gst_value_percentage AS GST ,order_items.sub_total AS Sell_Price,p_attr.manufacturer_price / p_attr.per_stript_qty AS M_price,shop_stocks.input_quantity AS Quantity,SUM(order_items.quantity) AS Sell_QTY,p_attr.per_stript_qty AS strip_qty FROM order_items inner JOIN orders ON(orders.order_id=order_items.order_id)inner JOIN product_attributes p_attr ON (p_attr.id=order_items.prod_id)inner JOIN sizes ON(sizes.id=p_attr.product_size)inner JOIN products ON(products.products_id=p_attr.products_id)inner JOIN gst_tax ON(gst_tax.gst_id=products.gst_id)inner JOIN shop_stocks ON(shop_stocks.shop_id=orders.shop_id) WHERE (orders.shop_id=101) GROUP BY(order_items.prod_id)"); 
    
       $data['increment_tax'] = DB::select("SELECT products.products_id,products.product_name,sizes.size_name,gst_tax.gst_value_percentage AS GST ,order_items.sub_total AS Sell_Price,p_attr.manufacturer_price / p_attr.per_stript_qty AS M_price,shop_stocks.input_quantity AS Quantity,SUM(order_items.quantity) AS Sell_QTY,p_attr.per_stript_qty AS strip_qty FROM order_items inner JOIN orders ON(orders.order_id=order_items.order_id)inner JOIN product_attributes p_attr ON (p_attr.id=order_items.prod_id)inner JOIN sizes ON(sizes.id=p_attr.product_size)inner JOIN products ON(products.products_id=p_attr.products_id)inner JOIN gst_tax ON(gst_tax.gst_id=products.gst_id)inner JOIN shop_stocks ON(shop_stocks.shop_id=orders.shop_id) WHERE (orders.shop_id=101 AND MONTH(order_items.created_at)=MONTH(CURRENT_DATE())-1 AND YEAR(order_items.created_at)=YEAR(CURRENT_DATE())) GROUP BY(order_items.prod_id)"); 
